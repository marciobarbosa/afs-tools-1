...
This optional directive can be used to add / alter the settings to be used when
contacting the database. For example, the admin can specify the timeout to use
when reading or connecting to the B<db/ro/dsn> database. For the MySQL driver,
...
In the following example, requests to the server will timeout after 5 seconds:
...
=item B<db/rw/options>

These are the same as their B<db/ro> equivalents, but for the read-write
database. If not specified, this directive will default to the value specified
in B<db/ro/options>.
